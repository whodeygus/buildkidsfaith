...
- To add a new month later: upload its files to the `curriculum` bucket following the same `month-NN/curriculum-pack.pdf` + `week-NN-story-slides.pptx` pattern, add matching entries to `config/curriculum-files.json`, and bump `TOTAL_MONTHS` in `index.html`'s `buildCatalog()` to match. Both the server-side gate (`config/total-months.js`) and the client-side display derive the month count from those two spots, so there's no separate list of files to hand-maintain.
